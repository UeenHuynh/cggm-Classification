...
def metadata_preprocessing(path_to_metadata,path_to_clinical_data, manifest_path):
    ...
    for i in tqdm(range(len(df3))):
        if i==0:
            continue
        # Normalize the file paths to remove any './'
        file_location = os.path.normpath(df3.iloc[i]['File Location'])
        previous_file_location = os.path.normpath(df3.iloc[i-1]['File Location'])
    ...
    def create_row(i, df4, flag=False):
        ...
        while j < 2:
            ...
            if not flag:
                if j == 0:
                    file_loc = os.path.normpath(str(df3.iloc[i, df3.columns.get_loc('File Location')]) + "/1-1.dcm")
                else:
                    file_loc = os.path.normpath(str(df3.iloc[i, df3.columns.get_loc('File Location')]) + "/1-2.dcm")
            if flag:
                if j == 0:
                    file_loc = os.path.normpath(str(df3.iloc[i, df3.columns.get_loc('File Location')]) + "/1-3.dcm")
                else:
                    file_loc = os.path.normpath(str(df3.iloc[i, df3.columns.get_loc('File Location')]) + "/1-4.dcm")
...
def stratify_data(df, manifest_path):
    ...
    def create_benign_malignant(df, dest_folder):
        ...
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):  
            src = (cmmd_dir+"manifest-1616439774456/"+row['file_location']).replace("datasetmanifest-", "dataset/manifest-")
...
def xception_transfer_learning(train_generator, validation_generator):
    ...
    history = model.fit(train_generator, epochs=25, callbacks=callbacks, validation_data=validation_generator)
...
def xception_fine_tune(train_generator, validation_generator):
    #load model
    model = keras.models.load_model("best_xception_fine_tuned_model.h5")
    ...
    model.compile(optimizer=keras.optimizers.Adam(learning_rate=0.000001),
              ...
              metrics=[keras.metrics.BinaryAccuracy()])
    history = model.fit(train_generator, epochs=25, callbacks=callbacks, validation_data=validation_generator)
    model.save('checkpoint_manual.h5')
    print("Checkpoint manually saved after fine-tuning")
